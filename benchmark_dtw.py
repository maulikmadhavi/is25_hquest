...
def benchmark_batch():
    ...
    for seq_len, num_seqs, query_len, label in test_cases:
        ...
        audio_sequences = [
            ",".join(str(int(x)) for x in np.random.uniform(1, 1000, seq_len))
        ...
        start = time.perf_counter()
        results = retrieve_dtw(query_tokens, audio_sequences, audio_filenames, top_k=5)
